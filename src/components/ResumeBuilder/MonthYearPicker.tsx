...
export const MonthYearPicker: React.FC<MonthYearPickerProps> = ({ value, onChange, label }) => {
  ...
  return (
    <div>
      ...
      <Button
        ...
        type="button"
      >
        {value ? `${months[Number(selectedMonth)-1]} ${year}` : "Select month and year"}
      </Button>
        ...
        <div className="p-4 w-64">
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            {months.map((m, i) => (
              <Button
                key={m}
                variant={selectedMonth === String(i+1).padStart(2, "0") ? "secondary" : "outline"}
                size="sm"
                className="w-full"
                onClick={() => {
          ...
          <div className="flex justify-between text-xs">
            <Button variant="link" size="sm" onClick={() => { onChange(""); setOpen(false); }}>Clear</Button>
